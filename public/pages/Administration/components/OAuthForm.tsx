      ...
      <Form error={error}>
        <div className="row">
          <div className="col-sm-9">
            <Input field="displayName" label="Display Name" maxLength={50} value={displayName} disabled={!fider.session.user.isAdministrator} onChange={setDisplayName} />

            <ImageUploader label="Logo" field="logo" bkey={logoBlobKey} previewMaxWidth={80} disabled={!fider.session.user.isAdministrator} onChange={handleLogoChange}>
              <p className="info">We accept JPG, GIF and PNG images, smaller than 50KB and with an aspect ratio of 1:1 with minimum dimensions of 24x24 pixels.</p>
            </ImageUploader>
          </div>
        ...
        />
        <Input field="authorizeURL" label="Authorize URL" maxLength={300} value={authorizeURL} disabled={!fider.session.user.isAdministrator} onChange={setAuthorizeURL} />
        ...
        <Input field="scope" label="Scope" maxLength={100} value={scope} disabled={!fider.session.user.isAdministrator} onChange={setScope}>
          <p className="info">
            It is recommended to only request the minimum scopes we need to fetch the user <strong>id</strong>, <strong>name</strong> and <strong>email</strong>. Multiple scopes must be separated by
            space.
          </p>
        </Input>
        ...
        <Input field="profileURL" label="Profile API URL" maxLength={300} value={profileURL} disabled={!fider.session.user.isAdministrator} onChange={setProfileURL}>
        ...
        <div className="row">
          <Input field="jsonUserIDPath" label="ID" className="col-sm-4" maxLength={100} value={jsonUserIDPath} disabled={!fider.session.user.isAdministrator} onChange={setJSONUserIDPath}>
            <p className="info">
              Path to extract User ID from the JSON. This ID <strong>must</strong> be unique within the provider or unexpected side effects might happen. For example below, the path would be{" "}
              <strong>id</strong>.
            </p>
          </Input>
          <Input field="jsonUserNamePath" label="Name" className="col-sm-4" maxLength={100} value={jsonUserNamePath} disabled={!fider.session.user.isAdministrator} onChange={setJSONUserNamePath}>
            <p className="info">
              Path to extract user Display Name from the JSON. This is optional, but <strong>highly</strong> recommended. For the example below, the path would be <strong>profile.name</strong>.
            </p>
          </Input>
          <Input field="jsonUserEmailPath" label="Email" className="col-sm-4" maxLength={100} value={jsonUserEmailPath} disabled={!fider.session.user.isAdministrator} onChange={setJSONUserEmailPath}>
            <p className="info">
              Path to extract user Email from the JSON. This is optional, but <strong>highly</strong> recommended. For the example below, the path would be <strong>profile.emails[0]</strong>.
            </p>
          </Input>
        </div>
        <pre>
        ...
        <div className="row">
          <div className="col-sm-4">
            <Field label="Status">
              ...
              {enabled && (
                <p className="info">
                  This provider will be available for everyone to use during the sign in process. It is recommended that you keep it disable and test it before enabling it. The Test button is
                  available after saving this configuration.
                </p>
              )}
