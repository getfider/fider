...
func generateXML(feed *AtomFeed) (string, error) {
	...
	feedXML, err := xml.MarshalIndent(feed, "", ident)
	if err != nil {
		return "", err
	}
...
/*
TODO:
	- test CORS outside of localhost
	- (comments) limit amount of comments? if yes, how to sort them?
	- ui
		- feed button
		- autodiscovery
	- test more feed readers
	- test upgrade from old version with privacy enabled - should not be a problem
	- unit tests (feed generator, settings)
*/
...
func GlobalFeed() web.HandlerFunc {
	return func(c *web.Context) error {
		...
		feed := &AtomFeed{
			Title:    c.Tenant().Name,
			Subtitle: Content{Body: c.Tenant().WelcomeMessage, Type: "html"},
			Id:       web.BaseURL(c),
			Link: []Link{
