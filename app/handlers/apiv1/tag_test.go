...
func TestCreateTagHandler_AlreadyInUse(t *testing.T) {
	...
	tag := &entities.Tag{Name: "Bug", Slug: "bug"}
	bus.AddHandler(func(ctx context.Context, q *query.GetTagBySlug) error {
		q.Result = tag
		return nil
	})
...
func TestEditExistingTagHandler(t *testing.T) {
	...
	tag := &entities.Tag{ID: 5, Name: "Bug", Slug: "bug", Color: "0000FF", IsPublic: true}
	bus.AddHandler(func(ctx context.Context, q *query.GetTagBySlug) error {
		q.Result = tag
		return nil
	})
...
func TestAssignTagHandler_Success(t *testing.T) {
	...
	post := &entities.Post{ID: 2, Number: 2}
	tag := &entities.Tag{ID: 5, Slug: "bug"}
...
func TestListTagsHandler(t *testing.T) {
	...
	tag1 := &entities.Tag{ID: 2, Name: "Bug", Slug: "bug", Color: "0000FF", IsPublic: true}
	tag2 := &entities.Tag{ID: 5, Name: "Feature Request", Slug: "feature-request", Color: "0000FF", IsPublic: true}
	bus.AddHandler(func(ctx context.Context, q *query.GetAllTags) error {
		q.Result = []*entities.Tag{tag1, tag2}
		return nil
	})
