...
func (u *dbUser) toModel(ctx context.Context) *entities.User {
	if u == nil {
		return nil
	}
	...
	user := &entities.User{
		...
		Providers:     make([]*entities.UserProvider, len(u.Providers)),
	...
	for i, p := range u.Providers {
		user.Providers[i] = &entities.UserProvider{
			Name: p.Name.String,
			UID:  p.UID.String,
		}
	}
...
func registerUser(ctx context.Context, c *cmd.RegisterUser) error {
	return using(ctx, func(trx *dbx.Trx, tenant *entities.Tenant, _ *entities.User) error {
		now := time.Now()
...
func queryUser(ctx context.Context, trx *dbx.Trx, filter string, args ...interface{}) (*entities.User, error) {
	user := dbUser{}
