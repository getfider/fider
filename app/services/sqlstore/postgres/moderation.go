...
func getModerationItems(ctx context.Context, q *query.GetModerationItems) error {
	return using(ctx, func(trx *dbx.Trx, tenant *entity.Tenant, user *entity.User) error {
		...
		err := trx.Select(&posts, `
			...
			WHERE p.tenant_id = $1 AND p.is_approved = false and p.status_slug <> $2
			ORDER BY p.created_at DESC`, tenant.ID, "deleted")
		if err != nil {
		...
		err = trx.Select(&comments, `
			...
			WHERE c.tenant_id = $1 AND c.is_approved = false and p.status_slug <> $2
			AND c.deleted_at IS NULL
			ORDER BY c.created_at DESC`, tenant.ID, "deleted")
		if err != nil {
...
func getModerationCount(ctx context.Context, q *query.GetModerationCount) error {
	return using(ctx, func(trx *dbx.Trx, tenant *entity.Tenant, user *entity.User) error {
		...
		err := trx.Get(&count, `
			SELECT
				(SELECT COUNT(*) FROM posts WHERE tenant_id = $1 AND is_approved = false and status_slug <> $2) +
				(SELECT COUNT(*) FROM comments c JOIN posts p on c.post_id = p.id WHERE p.tenant_id = $1 AND c.is_approved = false AND p.status_slug <> $2 AND c.deleted_at IS NULL)
		`, tenant.ID, "deleted")
