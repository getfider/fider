...
func (s *TenantStorage) Add(tenant *models.Tenant) error {
	row := s.DB.QueryRow(`INSERT INTO tenants (name, subdomain, created_on) 
						VALUES ($1, $2, $3) 
						RETURNING id`, tenant.Name, tenant.Subdomain, time.Now())
...
func (s *TenantStorage) First() (*models.Tenant, error) {
	...
	err := s.DB.Get(&tenant, "SELECT id, name, subdomain FROM tenants LIMIT 1")
